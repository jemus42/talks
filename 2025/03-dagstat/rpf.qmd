...
- Expand prediction $\hat{y}_i = \hat{m}(\mathbf{x}_i)$ into
    -   $\hat{m}_{0}$: Average prediction (_"intercept"_) plus...
...
(↑) Gains in interpretibility $\Rightarrow$ (↓) sacrifices in predictive performance?
...
Benchmark on **28** datasets [^ctr23] comparing RPF with _XGBoost_ & _RF_
...
- (↑) Feature _importance_ on same scale as target
- (↑) _Main-_ and _interaction effects_
