...
subtitle: "«CooPeR»"
author: "Lukas Burk¹²³, Andreas Bender²³, Marvin N. Wright¹"
...
- Typical approach: 
    ...
    - Treats other event as if censored
...
- $\theta$ is a scalar iff $\mathbf{Z} \in \mathbb{R}^{p \times 1}$
:::
...
- Simulation setup borrowed from @binder2009boostinghighdimensional, @binder2008adaptingprediction
- Context: Gene expression data with competing risk target
- n = 400, p = 5000, organized in 4 main blocks + 3000 noise variables
...
- Following Cox-exponential model (@bender2005generatingsurvival) for events $k = \{1,2\}$
...
- $\lambda_k = 0.1$ in default setting
- Censoring times analogously $C_i = -U_i/\lambda_C$ with $\lambda_C = 0.1$
- Observed event time is then $\min(T_i^{(1)}, T_i^{(2)}, C_i)$



## Assignments of True Effects
