...
tibble::tribble(
  ...
  kableExtra::row_spec(row = 4:5, color = RColorBrewer::brewer.pal(5, "Dark2")[[1]])
```
...
tibble::tribble(
 ...
 "XGBAFT" , "Gradient Boosting (AFT objective)",  "xgboost"
  ...
  kableExtra::row_spec(row = 2:3, color = RColorBrewer::brewer.pal(5, "Dark2")[[1]]) |>
  kableExtra::row_spec(row = 8:9, color = RColorBrewer::brewer.pal(5, "Dark2")[[4]])
```
...
bma |>
  filter(graf_improper < 1) |>
  dplyr::filter(tuned == "rcll") |>
  ggplot(aes(x = learner_id, y = graf_improper, color = learner_group, fill = learner_group)) +
  geom_boxplot(alpha = 1/4, key_glyph = "rect") +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  scale_color_manual(values = palette, aesthetics = c("color", "fill"), guide = guide_legend(keywidth = 2)) +
  labs(
...
<!-- ```{r} -->
<!-- #| out-height: "77%" -->
<!-- # knitr::include_graphics("img/boxplot-rcll-5.png") -->
<!-- knitr::include_graphics("img/boxplot-rcll.png") -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- #| out-height: "75%" -->
<!-- knitr::include_graphics("img/critical-difference-baseline-diff-rcll-4.png") -->
<!-- ``` -->
