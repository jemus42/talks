...
**18** learners implemented in R and available via `mlr3` [^mlr3]
...
- **Trees**: Individual, ensembles
- **Boosting**: Gradient- and likelihood
- **Other**: SVM
...
- Tuning space discussed with learner authors $\Rightarrow$ authoritative, equal opportunity
...
- CoxBoost [^coxboost] learner tunes itself with internal CV $\Rightarrow$ set to use 3 folds as well

[^coxboost]: [@pkgcoxboost]
...
- Main Result: Friedman rank sum tests and derived critical difference plots [^demsar]
...
- Results still need plausibility checks
- Cox regression: Still hard to beat
- The "standard setting" $\approx$ the "do you need ML?"-setting
- Only computationally possible due to the resources of the ARCC, UWYO [^beartooth]


[^beartooth]: {\small Advanced Research Computing Center (2023) Beartooth Computing Environment, x86_64 cluster. University of Wyoming, Laramie, WY}
