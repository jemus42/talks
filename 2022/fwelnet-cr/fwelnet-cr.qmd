...
::: columns
::: {.column width="50%"}
-   $\color{green}{\lambda} \in \mathbb{R}$ controls overall sparsity
...
- $\mathbf{z}_j^T$: Row $j$ of $\mathbf{Z}$ as row-vector
...
- Example from paper: $\mathbf{Z}$ set to noisy version of true $|\beta|$ in simulation study
...
- Setting with two causes: $(\mathbf{t}_1, \mathbf{y}_1), (\mathbf{t}_2, \mathbf{y}_2)$
- Assumption: Shared information for both causes:
    - If $\mathbf{x}_j$ is important for cause 1, may also be relevant for cause 2
